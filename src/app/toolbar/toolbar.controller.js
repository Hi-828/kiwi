(function ()
{
    ...
    /** @ngInject */
    function ToolbarController($rootScope, $mdSidenav, $translate, $mdToast, BendAuth, BendService, $location)
    {
        var vm = this;
        ...
        vm.user = BendAuth.getActiveUser();
      ...
      vm.avatar = "assets/images/avatars/profile.jpg";
      function getAvatar() {
        if(vm.user){
          BendService.getFile(vm.user.avatar, function(ret){
            $rootScope.globalData.data.userAvatar = ret;
            if(ret) {
              vm.avatar = ret._downloadURL;
            }
          })
        }
      }
      ...
      vm.goProfile = function() {
        $location.url("/users/" + vm.user._id + "/profile")
      }
